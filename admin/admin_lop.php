...
<body class="app header-fixed sidebar-fixed aside-menu-fixed sidebar-lg-show">
  ...
  <div class="app-body">
    <div class="sidebar">
      ...
      <nav class="sidebar-nav">
        <ul class="nav">
            ...
            <li class="nav-item">
              <a href="giangvien.php" class="nav-link">
                Quản lý Giảng viên
              </a>
            </li>
            <li class="nav-item">
    ...
    <main class="main">
    <button type="button" class="btn btn-success"><a href="themlop.php" style="color: white; text-decoration: none;">Thêm Lớp học</a></button>
      <table class="table table-striped">
        <tr>
          <th style="border-radius: 10px 0 0 0;">Mã lớp</th>
          <th>Mã môn học</th>
          <th>Id ngành</th>
          <th>Năm học</th>
          <th>Học kỳ</th>
        ...
        if(mysqli_num_rows($result)>0){
          while($row = mysqli_fetch_assoc($result))
          {
            ...
            echo'<td>'.$row['idnganh'].'</td>';
            echo'<td>'.$row['namhoc'].'</td>';
            echo'<td>'.$row['hocki'].'</td>';
            echo'<td>'.$row['idgiangvien'].'</td>';
            echo '<td>';
